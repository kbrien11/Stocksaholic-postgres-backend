...
# from flask_sqlalchemy import SQLAlchemy
from util import hash_pass,generate_key, crypto_market_cap,Crypto_Exchange_Data, seven_day_crypto_chart,chart, crypto_coins, Gemini_volume,Coinbase_volume,Binance_volume,get_price, generate_key,get_price_of_ticker, top_gainers,top_losers,crypto_news,usd_chart,crypto_stats,tracking_chart,Crypto,chart,crypto_chart
...
if ENV =='dev':
    ...
    app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:61Faa2GDEAae*b24D3BADC5E--1e2fEG@monorail.proxy.rlwy.net:49665/railway'
...
def equity_chart(api_key):
    user = db.session.query(Accounts).filter_by(api_key=api_key).first()
    result = user_schema.dump(user)
    string_user_pk = str(user.pk)
    if user:
        output = db.session.query(Date).filter_by(user_pk=string_user_pk).distinct(Date.unix_time).order_by(Date.unix_time.asc())
...
def delete_tracking(ticker,api_key):
    ...
    if user:
        data = request.get_json()
        ticker = data.get('ticker')
        tracking = db.session.query(Tracking).filter_by(user_pk = user.pk,ticker=ticker).all()
        for track in tracking:
            track.tracking = 0
            db.session.commit()
        return jsonify({"deleted":track.tracking})
    return jsonify({"error":"failed"})
